...
- Command line parameters
    ...
    ```
- Load network model and weights, activate gpu if needed, and compile model.
    ```
    checkpoint_fp = 'models/DAMDNet.pth.tar'
    arch = 'DAMDNet_v1'
    checkpoint = torch.load(checkpoint_fp, map_location=lambda storage, loc: storage)['state_dict']  #Loads an object saved with torch.save() from a file.
    model = getattr(DAMDNet, arch)(num_classes=62)  # 62 = 12(pose) + 40(shape) +10(expression)
    model_dict = model.state_dict()  # A Python dictionary object that maps each layer to its parameter tensor.
    for k in checkpoint.keys():  # Load weights
...
- ???
    ...
    transform = transforms.Compose([ToTensorGjz(), NormalizeGjz(mean=127.5, std=128)])  # Image transformations using TorchVision
    ````
- Capture image from webcam and get face bounding box.
    ```` 
    img_ori = cap.read()[1]
    imgScale = 1
    scaledImg = img_ori
...
- Network prediction
    ...
    ```` 
- Final face related predictions
    ```` 
    # 68 pts
