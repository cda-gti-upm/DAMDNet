...
def test_video(args):
    ...
    checkpoint = torch.load(checkpoint_fp, map_location=lambda storage, loc: storage)[
        'state_dict']  # Loads an object saved with torch.save() from a file.
    ...
    while True:
        # get a frame
        img_ori = cap.read()[1]
        #img_ori = cv2.imread('imgs/obama.jpg')  # For testing
        imgScale = 1
        scaledImg = img_ori
        ...
        for rect in rects:  # Only the last face is processed?
            if args.dlib_landmark:
